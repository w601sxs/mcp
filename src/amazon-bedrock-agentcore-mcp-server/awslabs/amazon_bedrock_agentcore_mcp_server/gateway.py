...
async def _find_and_upload_smithy_model(smithy_model: str, region: str, setup_steps: List[str]) -> Optional[str]:
    """
    ...
    try:
        ...
        setup_steps.append(f"Network: Searching GitHub API for {smithy_model} service directory...")
        
        github_api_url = f"https://api.github.com/repos/aws/api-models-aws/contents/models/{smithy_model}"
        ...
        setup_steps.append(f"OK Found service directory")
        ...
        if response.status_code != 200:
            setup_steps.append(f"X Could not access service directory")
            return None
        ...
        if response.status_code != 200:
            setup_steps.append(f"X Could not access version directory")
            return None
        ...
        setup_steps.append(f"Download: Downloading Smithy model JSON...")
        
        response = requests.get(download_url)
        if response.status_code != 200:
            setup_steps.append(f"X Could not download service.json")
            return None
        ...
        try:
            ...
            setup_steps.append(f"OK Uploaded Smithy model to S3")
            ...
            for category, patterns in category_patterns.items():
                if any(pattern in service.lower() for pattern in patterns):
                    categories[category].append({
                        "name": service,
                        "description": f"AWS {service} service"
                    })
                    categorized = True
                    break
...
async def _upload_openapi_schema(openapi_spec: dict, gateway_name: str, region: str, setup_steps: List[str], api_key: str = "", credential_location: str = "QUERY_PARAMETER", credential_parameter_name: str = "api_key") -> Optional[dict]:
    """
    ...
    try:
        ...
        try:
            ...
            setup_steps.append(f"OK Uploaded OpenAPI schema to S3")
            ...
            if api_key:
                setup_steps.append(f"Security: Configuring API key credentials ({credential_location})")
                ...
                setup_steps.append(f"OK API key credential configuration created")
...
def register_gateway_tools(mcp: FastMCP):
    ...
    async def agent_gateway(
        action: str = Field(default="setup", description="Gateway action", enum=[
            "setup", "create", "targets", "cognito", "list", "delete", "auth", "test", 
            "list_tools", "search_tools", "invoke_tool", "discover", "safe_examples"
        ]),
        gateway_name: str = Field(default="", description="Gateway name"),
        target_type: str = Field(default="lambda", description="Target type", enum=["lambda", "openApiSchema", "smithyModel"]),
        smithy_model: str = Field(default="", description="AWS Smithy model name (e.g., 's3', 'dynamodb')"),
        ...
        enable_semantic_search: bool = Field(default=True, description="Enable semantic search for tools"),
        credentials: dict = Field(default=None, description="Authentication credentials"),
        credential_location: str = Field(default="QUERY_PARAMETER", description="Where to include credentials"),
        credential_parameter_name: str = Field(default="api_key", description="Parameter name for credentials"),
...
"""
                    ...
                    # Format results
                    result_parts = []
                    result_parts.append(f"# Gateway: Gateway Resources Found")
                    ...
                    for gateway in gateways:
                        ...
                        result_parts.append(f"- Delete: `agent_gateway(action=\"delete\", gateway_name=\"{gateway_name}\")`")
                        
                    result_parts.append("")
                    result_parts.append("## Gateway Operations:")
                    result_parts.append("- Delete Gateway: `agent_gateway(action=\"delete\", gateway_name=\"GATEWAY_NAME\")` ! Permanent")
                    result_parts.append("- Test Gateway: `agent_gateway(action=\"test\", gateway_name=\"GATEWAY_NAME\")`")
                    result_parts.append("")
                    result_parts.append("## AWS Console Links:")
                    result_parts.append(f"- [Bedrock AgentCore Console](https://console.aws.amazon.com/bedrock/home?region={region}#/agentcore/gateways)")
                    result_parts.append(f"- [Cognito Console](https://console.aws.amazon.com/cognito/v2/home?region={region})")
            ...
            # Action: delete - Delete gateway with robust target cleanup
                ...
                try:
                    ...
                    try:
                        ...
                        deletion_steps.append(f"List: Listed gateway targets: {len(targets)} found")
                    ...
                    if targets:
                        ...
                        remaining_target_ids = [t.get('targetId') for t in targets if t.get('targetId')]
                        max_deletion_attempts = 5
                        
                        for attempt in range(max_deletion_attempts):
                            deletion_steps.append(f"Retry: Deletion attempt {attempt + 1}/{max_deletion_attempts}")
                            
                            # Track which targets were successfully deleted in this attempt
                            successfully_deleted = []
                            
                            for target_id in remaining_target_ids:
                                target_name = next((t.get('name', 'unknown') for t in targets if t.get('targetId') == target_id), 'unknown')
                                
                                try:
                                    deletion_steps.append(f"Delete: Deleting target: {target_name} ({target_id})")
                                    client.delete_gateway_target(
                                        gatewayIdentifier=gateway_id,
                                        targetId=target_id
                                    )
                                    deletion_steps.append(f"OK Successfully deleted target: {target_name}")
                                    ...
                                    if 'throttling' in error_msg or 'rate' in error_msg:
                                        deletion_steps.append(f"Throttling: Throttling detected for {target_name}, will retry after delay")
                                        # Wait longer for throttling
                                        time.sleep(10)
                                    elif 'not found' in error_msg or 'does not exist' in error_msg:
                                        deletion_steps.append(f"OK Target {target_name} already deleted")
                                        successfully_deleted.append(target_id)
                                    else:
                                        deletion_steps.append(f"X Failed to delete target {target_name}: {str(target_error)}")
                            
                            # Remove successfully deleted targets from remaining list
                            remaining_target_ids = [tid for tid in remaining_target_ids if tid not in successfully_deleted]
                            ...
                            if attempt < max_deletion_attempts - 1:
                                wait_time = min(30, (attempt + 1) * 10)  # Exponential backoff, max 30s
                                deletion_steps.append(f"Wait: {len(remaining_target_ids)} targets remaining. Waiting {wait_time} seconds before retry...")
                                ...
                                try:
                                    ...
                                    current_target_ids = [t.get('targetId') for t in current_targets if t.get('targetId')]
                                    
                                    # Only retry targets that actually still exist
                                    remaining_target_ids = [tid for tid in remaining_target_ids if tid in current_target_ids]
                                    
                                    if not remaining_target_ids:
                                        deletion_steps.append("OK All targets confirmed deleted via verification")
                                        break
                                        
                                    deletion_steps.append(f"List: Verified {len(remaining_target_ids)} targets still exist")
                                    
                                except Exception as verify_error:
                                    deletion_steps.append(f"! Could not verify remaining targets: {str(verify_error)}")
                        ...
                        if remaining_target_ids:
                            ...
                            try:
                                final_verify_response = client.list_gateway_targets(gatewayIdentifier=gateway_id)
                                final_remaining_targets = final_verify_response.get('items', [])
                                final_remaining_ids = [t.get('targetId') for t in final_remaining_targets if t.get('targetId')]
                                
                                if not final_remaining_ids:
                                    deletion_steps.append("OK Final verification: All targets successfully deleted")
                                else:
                                    # Return detailed error for manual intervention
                                    deletion_steps.append(f"X {len(final_remaining_ids)} targets could not be deleted after {max_deletion_attempts} attempts")
...
AWS Console: [Bedrock AgentCore Console](https://console.aws.amazon.com/bedrock/home?region={region}#/agentcore/gateways)"""
                                ...
                                deletion_steps.append(f"X Could not perform final verification: {str(final_verify_error)}")
...
Manual Action Required: Use AWS Console to check and complete deletion"""
                    ...
                    for gateway_attempt in range(gateway_deletion_attempts):
                        try:
                            deletion_steps.append(f"Delete: Attempting to delete gateway: {gateway_name} (attempt {gateway_attempt + 1}/{gateway_deletion_attempts})")
                            client.delete_gateway(gatewayIdentifier=gateway_id)
                            deletion_steps.append("OK Gateway deletion successful")
                            ...
                            if 'throttling' in error_msg or 'rate' in error_msg:
                                deletion_steps.append(f"Throttling: Gateway deletion throttled, waiting before retry...")
                                ...
                                deletion_steps.append(f"X Gateway still has associated targets - this should not happen after target cleanup")
                                # Wait a bit longer and try once more
                                if gateway_attempt < gateway_deletion_attempts - 1:
                                    deletion_steps.append("Wait: Waiting 30 seconds for AWS to fully process target deletions...")
                                    time.sleep(30)
                                    continue
                            
                            deletion_steps.append(f"X Gateway deletion attempt {gateway_attempt + 1} failed: {str(delete_error)}")
...
```"""
                    ...
                    result_parts.append(f"Target: Total Services Found: {sum(len(services) for services in discovered_services.values())}")
                    result_parts.append("")
                    ...
                    for category, services in discovered_services.items():
                        if services:  # Only show categories with services
                            ...
                            for service in services:
                                result_parts.append(f"- {service['name']} - {service['description']}")
                            result_parts.append("")
                    ...
                    if available_popular:
                        for service in available_popular[:3]:  # Show top 3
                            ...
                            result_parts.append(f"agent_gateway(")
                            result_parts.append(f"    action=\"setup\",")
                    ...
                    result_parts.append("    api_key=\"your-api-key\"")
                    result_parts.append(")")
                    result_parts.append("```")
                    result_parts.append("")
                    ...
                    result_parts.append("agent_gateway(action=\"setup\", gateway_name=\"my-gateway\", smithy_model=\"SERVICE_NAME\")")
                    result_parts.append("```")
                    result_parts.append("")
                    result_parts.append("OK All services shown are verified to exist in the AWS API models repository!")
                    ...
                    if smithy_model or openapi_spec:
                        ...
                        try:
                            if smithy_model:
                                ...
                                setup_steps.append(f"Search: Searching for {smithy_model} Smithy model in GitHub...")
                                
                                try:
                                    # Search for the Smithy model in AWS API models repository
                                    smithy_s3_uri = await _find_and_upload_smithy_model(smithy_model, region, setup_steps)
                                    
                                    if not smithy_s3_uri:
                                        raise Exception(f"Could not find or upload Smithy model for {smithy_model}")
                                    
                                    setup_steps.append(f"OK Smithy model uploaded to: {smithy_s3_uri}")
                                    
                                    # Create proper smithy model configuration with S3 URI
                                    smithy_target_config = {
                                        "s3": {
                                            "uri": smithy_s3_uri
                                        }
                                    }
                                    
                                    setup_steps.append(f"Target: Creating Smithy target with S3 URI...")
                                    ...
                                    setup_steps.append(f"X Smithy model setup failed: {str(smithy_error)}")
...
Gateway URL: Check AWS Console for connection details"""
                                ...
                                setup_steps.append(f"OK Smithy target '{smithy_model}' added successfully")
                                ...
                                setup_steps.append(f"Document: Processing OpenAPI schema for {gateway_name}...")
                                ...
                                try:
                                    ...
                                    if not openapi_result:
                                        raise Exception("Could not upload OpenAPI schema or configure credentials")
                                    
                                    setup_steps.append(f"OK OpenAPI schema uploaded to: {openapi_result['s3_uri']}")
                                    
                                    # Create proper OpenAPI target configuration
                                    openapi_target_config = {
                                        "s3": {
                                            "uri": openapi_result['s3_uri']
                                        }
                                    }
                                    ...
                                    if openapi_result['credential_config']:
                                        credentials_config.append(openapi_result['credential_config'])
                                        setup_steps.append(f"Security: API key credentials configured")
                                    
                                    setup_steps.append(f"Target: Creating OpenAPI target with S3 URI...")
                                    ...
                                    target_result = client.create_mcp_gateway_target(
                                        ...
                                        credentials=credentials_config if credentials_config else None
                                    )
                                    
                                except Exception as openapi_error:
                                    setup_steps.append(f"X OpenAPI target setup failed: {str(openapi_error)}")
                                    # Continue without target - gateway still created
                                    return f"""! Gateway Created with Warnings
...
Gateway URL: Check AWS Console for connection details"""
                    ...
                    while time.time() - wait_start < max_wait_time:
                        try:
                            ...
                            gateway_status = client.client.get_gateway(gatewayIdentifier=gateway_result['gateway']['gatewayId'])
                        ...
                        setup_steps.append("Time: Gateway still initializing - may take a few more minutes")
...
{f"- Targets: OpenAPI specification added" if openapi_spec else ""}
...
2. Recreate configuration: `agent_gateway(action="setup", gateway_name="{gateway_name}")`"""
                    ...
                    tools_data = []
                    if hasattr(result, 'get') and result.get('structuredContent') and result['structuredContent'].get('tools'):
                        ...
                        tools_data = [{"name": tool, "description": "Found via semantic search"} for tool in set(tool_matches)]
