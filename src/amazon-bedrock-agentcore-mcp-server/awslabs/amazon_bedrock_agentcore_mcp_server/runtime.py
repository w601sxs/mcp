...
from .utils import (
    SDK_AVAILABLE, RUNTIME_AVAILABLE, SDK_IMPORT_ERROR,
    resolve_app_file_path, get_user_working_directory, get_runtime_for_agent,
    find_agent_config_directory, check_agent_config_exists, get_agentcore_command,
    analyze_code_patterns, format_patterns, format_dependencies, format_features_added,
    YAML_AVAILABLE
)
...
def check_agent_oauth_status(agent_name: str, region: str = "us-east-1"):
    ...
    try:
                        ...
                        if f"{agent_name}:" in yaml_content and "agent_arn:" in yaml_content:
                            ...
                            for line in lines:
                                if f"{agent_name}:" in line:
                                    ...
                                    break
                                elif in_agent_section and not line.startswith(' ') and line.strip():
        ...
        try:
            runtime_response = agentcore_client.get_agent_runtime(agentRuntimeArn=agent_runtime_arn)
            ...
            if oauth_deployed:
                ...
                if 'customJWTAuthorizer' in inbound_config:
                    ...
                    auth_details.append(f"JWT Auth (Client: {jwt_auth.get('clientId', 'Unknown')})")
                if 'cognitoAuthorizer' in inbound_config:
                    cognito_auth = inbound_config['cognitoAuthorizer']
                    auth_details.append(f"Cognito Auth (Pool: {cognito_auth.get('userPoolId', 'Unknown')})")
                
                auth_summary = ", ".join(auth_details) if auth_details else "Custom auth configured"
                return True, oauth_available, f"Agent deployed with OAuth: {auth_summary}"
            
            elif oauth_available:
                return False, True, f"Agent deployed without OAuth, but OAuth config available for redeployment"
            ...
            if oauth_available:
                return False, True, f"Cannot verify deployment OAuth status (API error: {str(api_error)}), but OAuth config exists locally"
            else:
        ...
        if oauth_available:
            return False, True, f"Cannot verify deployment OAuth status ({str(e)}), but OAuth config exists locally"
        else:
...
def validate_oauth_config(agent_name: str, region: str = "us-east-1"):
    ...
    try:
        ...
        oauth_deployed, oauth_available, oauth_status = check_agent_oauth_status(agent_name, region)
        ...
        if not config_file.exists():
            return False, f"""X OAuth Agent Configuration Not Found
...
"""
        ...
        try:
            ...
            if not client_info:
                return False, f"""X Invalid OAuth Configuration
...
"""
            ...
            if missing_fields:
                return False, f"""X Incomplete Cognito Configuration
...
"""
            ...
            return False, f"""X Invalid JSON Configuration
...
"""
        ...
        return False, f"""X OAuth Config Validation Error: {str(e)}
...
def generate_oauth_token(client_info: dict, region: str = "us-east-1"):
    ...
    try:
        ...
        return False, f"""X Missing Dependencies: {str(import_error)}
...
"""
        ...
        return False, f"""X Token Generation Failed: {str(token_error)}
...
def register_analysis_tools(mcp: FastMCP):
    ...
    async def analyze_agent_code(
        file_path: str = Field(description="Path to your agent file (e.g., 'agent.py', 'main.py')"),
        code_content: str = Field(default="", description="Optional: Paste code directly if no file")
    ) -> str:
...
"""
    ...
    async def transform_to_agentcore(
        ...
        target_file: str = Field(default="", description="Output file name (auto-generated if empty)"),
        ...
        add_tools: bool = Field(default=False, description="Add code interpreter and browser tools (optional)")
    ) -> str:
...
"""
            ...
            transformed_code = generate_safe_agentcore_code(original_code, analysis, {
...
def register_deployment_tools(mcp: FastMCP):
    ...
    async def deploy_agentcore_app(
        ...
        execution_role: str = Field(default="auto", description="IAM execution role (auto to create)"),
        environment: str = Field(default="dev", description="Deployment environment"),
        enable_oauth: bool = Field(default=False, description="Enable OAuth authentication for agent invocation"),
        cognito_user_pool: str = Field(default="", description="Existing Cognito User Pool ID (or auto-create)")
    ) -> str:
...
"""
                ...
                deployment_result = await execute_agentcore_deployment_cli(
                    app_file, agent_name, region, memory_enabled, execution_role, environment,
                    enable_oauth, cognito_user_pool
                )
            else:  # sdk
    ...
    async def invoke_agent(
        ...
        if not RUNTIME_AVAILABLE:
            return f"""X Runtime Not Available
                    ...
                    if isinstance(response_data, list) and len(response_data) > 0:
                        if isinstance(response_data[0], bytes):
                            try:
                                ...
                                response_data = decoded_response if 'decoded_response' in locals() else str(response_data[0])
                    elif isinstance(response_data, bytes):
                        try:
                            decoded_response = response_data.decode('utf-8')
                            response_data = json.loads(decoded_response)
                        except (UnicodeDecodeError, json.JSONDecodeError):
                            response_data = decoded_response if 'decoded_response' in locals() else str(response_data)
                else:
...
Note: Alternative: Try `invoke_agent` instead - your agent may work without OAuth authentication."""
            ...
            try:
                if not RUNTIME_AVAILABLE:
                    return f"""X Runtime SDK not available
...
"""
                    ...
                    try:
                        ...
                        if hasattr(result, 'get') and 'response' in result:
                            ...
                            if isinstance(response_data, list) and len(response_data) > 0:
                                if isinstance(response_data[0], str) and response_data[0].startswith("b'"):
                                    # Handle byte string format
                                    try:
                                        # Remove b' and ' wrapper, then parse JSON
                                        json_str = response_data[0][2:-1].replace('\\"', '"').replace('\\\\n', '\\n')
...
"""
    ...
    async def check_oauth_status(
        ...
        try:
            ...
            oauth_deployed, oauth_available, oauth_message = check_agent_oauth_status(agent_name, region)
...
#### Next Steps:
{recommendation}
{'' if oauth_deployed or not oauth_available else '''
...
"""
    ...
    async def invoke_agent_smart(
        agent_name: str = Field(description="Agent name to invoke (tries regular first, OAuth if needed)"),
        prompt: str = Field(description="Message to send to agent"),
        session_id: str = Field(default="", description="Session ID for conversation continuity"),
        region: str = Field(default="us-east-1", description="AWS region")
    ) -> str:
        ...
        try:
            ...
            try:
                regular_result = await invoke_agent(agent_name=agent_name, prompt=prompt, session_id=session_id, region=region)
                
                # If regular invocation succeeded, return with note
                if "Agent: Agent Invocation Successful" in regular_result:
                    return regular_result.replace(
...
"""
    ...
    async def get_agent_status(
        ...
        try:
            ...
            if config_exists:
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=30, cwd=config_dir)
            else:
...
"""
    ...
    async def discover_existing_agents(
        search_path: str = Field(default=".", description="Path to search for existing agent configurations"),
        include_status: bool = Field(default=True, description="Check deployment status of found agents")
    ) -> str:
...
"""
            ...
            for config_file in config_files:
                try:
                                ...
                                # Handle complex config format (multi-agent)
                                    ...
                                    for agent_name, agent_config in config['agents'].items():
                                        multi_agent_info = {
                                            ...
                                            'agent_id': agent_config.get('bedrock_agentcore', {}).get('agent_id', 'unknown'),
                                            'agent_arn': agent_config.get('bedrock_agentcore', {}).get('agent_arn', 'unknown')
                                        }
                    ...
                    if agent_info.get('config_format') == 'simple' or 'config_format' not in agent_info:
                        discovered_agents.append(agent_info)
...
"""
            ...
            # Format results
            result_parts = []
            result_parts.append(f"# Search: Discovered {len(discovered_agents)} Agent Configuration(s)")
            result_parts.append("")
            ...
            if ready_agents:
                ...
                for agent in ready_agents:
                    ...
                    result_parts.append(f"- Invoke: `invoke_agent` with agent_name: `{agent['name']}`")
                    result_parts.append("")
            
            if configured_agents:
                result_parts.append(f"### Configured: Configured But Not Deployed ({len(configured_agents)} agents):")
                for agent in configured_agents:
                    result_parts.append(f"#### {agent['name']}")
                    result_parts.append(f"- Status: {agent['status']}")  
                    result_parts.append(f"- Config: `{agent['config_file']}`")
                    result_parts.append(f"- Deploy: Use `deploy_agentcore_app` with execution_mode: `sdk`")
                    result_parts.append("")
...
def generate_safe_agentcore_code(original_code: str, analysis: Dict[str, Any], options: Dict[str, bool]) -> str:
...
def generate_basic_agentcore_code(original_code: str, analysis: Dict[str, Any], options: Dict[str, bool]) -> str:
...
async def execute_agentcore_deployment_cli(
    app_file: str, agent_name: str, region: str, 
    memory_enabled: bool, execution_role: str, environment: str,
    enable_oauth: bool = False, cognito_user_pool: str = ""
) -> str:
...
"""
        ...
        if enable_oauth:
            deployment_steps.append("Security: OAuth authentication enabled - setting up Cognito...")
            ...
            try:
                ...
                cognito_result = gateway_client.create_oauth_authorizer_with_cognito(f"{agent_name}_runtime")
                client_info = cognito_result.get('client_info', {})
                
                deployment_steps.append(f"OK Cognito User Pool: {client_info.get('user_pool_id', 'Unknown')}")
                ...
                deployment_steps.append("OK Config format compatible with existing `get_oauth_access_token` tool")
...
"""
        ...
        try:
            ...
            while time.time() - wait_start < max_wait_time:
                ...
                deployment_steps.append("Timeout: Deployment timeout - agent may still be starting")
            ...
            if deployment_results.get("status") == "READY":
                ...
                try:
                    ...
                    if enable_oauth:
                        ...
                        deployment_steps.append("Note: Use `get_runtime_oauth_token` and `invoke_oauth_agent` for testing")
...
- Use `get_agent_status` to monitor agent health
{f"- Use `get_runtime_oauth_token` for OAuth token generation" if enable_oauth else ""}
...
async def invoke_agent_via_aws_sdk(agent_name: str, prompt: str, session_id: str, region: str) -> str:
