...
from .utils import (
    ...
    resolve_app_file_path, find_agent_config_directory, YAML_AVAILABLE
)
...
No relevant memories found for this query."""
            ...
            for i, memory in enumerate(memories, 1):
                ...
                result_parts.append(f"- **Content**: {content}{'...' if len(str(memory.get('content', ''))) > 200 else ''}")
                result_parts.append("")
...
No conversation history found for this session."""
            ...
            for i, turn in enumerate(conversation_turns, 1):
                ...
                for j, message in enumerate(turn):
                    ...
                    result_parts.append(f"- **{role.title()}**: {content}{'...' if len(str(message.get('content', ''))) > 150 else ''}")
...
- Session ID: {session_id}"""
    ...
    async def memory_process_turn(
        ...
        user_input: str = Field(description="User message"),
        agent_response: str = Field(description="Agent response"),
        retrieval_namespace: str = Field(default="", description="Namespace for retrieval (optional)"),
        ...
        try:
            ...
            result_parts.append(f"## 💾 Stored Turn")
...
No events found for this session."""
            ...
            for i, event in enumerate(events, 1):
                ...
                if payload:
                    ...
                    result_parts.append(f"- **Payload**: {content}{'...' if len(str(payload)) > 150 else ''}")
...
- Session ID: {session_id}"""
    ...
    async def memory_create_event(
        ...
        actor_id: str = Field(description="Actor/user ID"), 
        session_id: str = Field(description="Session ID"),
        messages: List[str] = Field(description="List of messages as ['role:content', 'role:content']")
    ) -> str:
        """
...
- Messages: {messages}"""
    ...
    async def agent_memory(
        action: str = Field(description="Memory action", enum=[
            "create", "list", "health", "delete"
        ]),
        ...
        strategy_types: List[str] = Field(default=["semantic", "summary"], description="Memory strategy types"),
...
[Bedrock AgentCore Console](https://console.aws.amazon.com/bedrock/home?region={region}#/agentcore/memories)"""
                    ...
                    result_parts.append("- **Create**: `agent_memory(action=\"create\", agent_name=\"my-agent\")`")
                    result_parts.append("- **Health Check**: `agent_memory(action=\"health\", memory_id=\"MEMORY_ID\")`")
                    result_parts.append("- **Delete**: `agent_memory(action=\"delete\", memory_id=\"MEMORY_ID\")`")
...
```"""
                ...
                try:
                    ...
                    if sanitized_agent_name != agent_name:
                        create_steps.append(f"🔧 Fixed agent name: '{agent_name}' → '{sanitized_agent_name}' (replaced - with _)")
                    ...
                    for strategy_name in strategy_types:
                        ...
                        if strategy_key:
                            ...
                            create_steps.append(f"📝 Added {strategy_name} strategy: {strategy_dict}")
                    
                    if not memory_strategies:
                        # Default to semantic if no valid strategies - same as test
                        memory_strategies = [{"semanticMemoryStrategy": {"name": "semantic_strategy"}}]
                    ...
                    create_result = memory_control.create_memory(
                        name=f"{sanitized_agent_name}_memory",
                    ...
                    while time.time() - wait_start < max_wait_time:
                        ...
                        create_steps.append("⏰ Memory still initializing - may take a few more minutes")
                    
                    # Optional: Integrate with agent file if provided
                    integration_result = ""
                    if agent_file:
                        create_steps.append(f"🔗 Integrating memory with agent file: **{agent_file}**")
                        
                        # Resolve agent file path
                        resolved_file = resolve_app_file_path(agent_file)
                        if not resolved_file:
                            create_steps.append(f"⚠️ Agent file not found: {agent_file}")
                            integration_result = f"\n⚠️ Memory created but integration skipped - file not found: **{agent_file}**"
                        else:
                            try:
                                # Read agent code
                                ...
                                if 'MemoryClient' in agent_code and memory_id in agent_code:
                                    ...
                                    for i, line in enumerate(code_lines):
                                        if 'from bedrock_agentcore' in line and not import_added:
                                            code_lines.insert(i + 1, "from bedrock_agentcore.memory import MemoryClient")
                                            import_added = True
                                            break
                                    
                                    if not import_added:
                                        code_lines.insert(0, "from bedrock_agentcore.memory import MemoryClient")
                                    
                                    # Add memory client initialization
                                    for i, line in enumerate(code_lines):
                                        if 'BedrockAgentCoreApp()' in line:
                                            code_lines.insert(i + 1, f"memory_client = MemoryClient(memory_id=\"{memory_id}\")")
                                            break
                                    ...
                                    create_steps.append(f"✅ Memory integrated with **{resolved_file}**")
...
- **Memory Client**: Ready to use"""
                                ...
                                create_steps.append(f"⚠️ Integration failed: {str(integration_error)}")
                                integration_result = f"\n⚠️ Memory created but integration failed: **{str(integration_error)}**"
                ...
                try:
                    ...
                    if is_healthy:
                        ...
                        try:
                            ...
                            health_steps.append(f"⚠️ Memory client test failed: {str(client_error)}")
                    ...
                    # Format strategies
                    strategy_names = [s.get('strategyType', 'unknown') for s in strategies] if strategies else ['none']
...
To find memory IDs: `agent_memory(action="list")`"""
                ...
                try:
                    ...
                    return f"""# 🗑️ Memory Deleted Successfully
...
- No action required"""
            ...
            return """⚠️ Memory is active but client connectivity has issues
...
def get_memory_health_next_steps(is_healthy: bool, memory_id: str) -> str:
    ...
    if is_healthy:
        return f"""- **Use Memory**: Integrate with agent code and deploy
...
- **Scale**: Add more memory strategies if needed"""
        ...
        return f"""- **Troubleshoot**: Address the issues identified above
