...
class TestIntegrationTests:
    ...
    async def test_server_full_lifecycle(self):
        ...
        env_result_tuple = await mcp.call_tool("validate_agentcore_environment", {
            "project_path": "."
        })
    ...
    async def test_oauth_tools_integration(self):
        ...
        oauth_result_tuple = await mcp.call_tool("get_oauth_access_token", {
            "method": "ask"
        })
...
'''
        ...
        result_tuple = await mcp.call_tool("analyze_agent_code", {
            "file_path": "",
    ...
    async def test_discovery_tools_integration(self):
        ...
        discovery_result_tuple = await mcp.call_tool("project_discover", {
            "action": "all",
            "search_path": "."
        })
        ...
        assert ("Examples Discovery" in examples_result or 
                "Error" in examples_result or
                "GitHub" in examples_result)
...
class TestErrorHandlingIntegration:
    ...
    async def test_tools_with_missing_files(self):
        ...
        result_tuple = await mcp.call_tool("analyze_agent_code", {
            "file_path": "definitely_does_not_exist.py"
        })
        result = extract_result(result_tuple)
    ...
    async def test_tools_with_invalid_regions(self):
        ...
        result_tuple = await mcp.call_tool("agent_gateway", {
            "action": "list",
    ...
    async def test_deployment_without_aws_creds(self):
        ...
        with patch('boto3.client') as mock_client:
            ...
            assert result is not None
            assert ("Error" in result or "credentials" in result.lower() or 
                   "not found" in result.lower() or "not available" in result.lower())
...
class TestMainFunction:
    ...
    def test_main_entry_point_import(self):
        ...
        from awslabs.amazon_bedrock_agentcore_mcp_server.server import run_main, mcp
...
if __name__ == "__main__":
    ...
    async def run_integration_tests():
        ...
        result = extract_result(result_tuple)
        print("✓ Server info integration test passed")
        ...
        env_result = extract_result(env_result_tuple)
        print("✓ Environment validation integration test passed")
