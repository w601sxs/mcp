...
class TestAgentCoreMCPServer:
    ...
    async def test_tools_registration(self):
        ...
        # Discovery tools
        assert "what_agents_can_i_invoke" in tool_names
    ...
    async def test_validate_agentcore_environment_tool(self):
        ...
        with patch('awslabs.amazon_bedrock_agentcore_mcp_server.utils.get_user_working_directory') as mock_dir:
            from pathlib import Path
            mock_dir.return_value = Path.cwd()
            
            result_tuple = await mcp.call_tool("validate_agentcore_environment", {
                "project_path": ".",
                "check_existing_agents": False
            })
            result = extract_result(result_tuple)
...
class TestErrorHandling:
    ...
    async def test_tool_with_invalid_params(self):
        ...
        assert result is not None
        assert ("not found" in result.lower() or "error" in result.lower() or "missing" in result.lower() or "not available" in result.lower())
...
class TestSDKAvailability:
    ...
    async def test_tools_handle_missing_sdk(self):
        ...
        with patch('awslabs.amazon_bedrock_agentcore_mcp_server.utils.SDK_AVAILABLE', False), \
            ...
            assert result is not None
            assert "SDK Not Available" in result or "not available" in result.lower()
