...
def check_agent_ready_for_invocation(agent_name: str, region: str = "us-east-1") -> Tuple[bool, str, Optional[str]]:
...
def wait_for_agent_ready(agent_name: str, region: str = "us-east-1", timeout_minutes: int = 2) -> Tuple[bool, str, Dict[str, Any]]:
            ...
            for agent in agents.get('agentRuntimes', []):
                if agent.get('agentRuntimeName', agent.get('name')) == agent_name:
                    ...
                    if status == 'READY':
                        ...
                        return False, f"Agent {agent_name} deployment failed with status: {status}", agent
    ...
    # Timeout reached
    return False, f"Timeout waiting for agent {agent_name} to be ready (waited {timeout_minutes} minutes)", {}
