...
async def test_strandsv2_workflow():
    """Test the complete workflow: list -> find -> invoke strandsv2"""
    ...
    try:
        ...
        try:
            result = await server.call_tool('project_discover', {
                'action': 'aws',
                'region': 'us-east-1'
            })
        ...
        try:
            result = await server.call_tool('get_agent_status', {
        ...
        try:
            result = await server.call_tool('invoke_agent', {
                'agent_name': 'strandsv2',
