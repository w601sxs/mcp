#!/bin/bash
# Script to run the bedrock-kb-retrieval-mcp-server tests
...
if [ ${#MISSING_PACKAGES[@]} -gt 0 ]; then
    ...
    for pkg in "pytest" "boto3" "loguru" "pydantic" "mcp"; do
...
# Check if bedrock_kb_retrieval_mcp_server module can be imported
echo "Checking if bedrock_kb_retrieval_mcp_server module can be imported..."
python -c "import awslabs.bedrock_kb_retrieval_mcp_server; print(f'bedrock_kb_retrieval_mcp_server module found at: {awslabs.bedrock_kb_retrieval_mcp_server.__file__}')" 2>/dev/null
if [ $? -ne 0 ]; then
    echo "Warning: bedrock_kb_retrieval_mcp_server module cannot be imported. This may cause test failures."
    ...
    # Check if pip is available
        ...
        ln -sf $(pwd)/awslabs/bedrock_kb_retrieval_mcp_server $(pwd)/bedrock_kb_retrieval_mcp_server 2>/dev/null
    fi

    echo "Trying again..."
    python -c "import awslabs.bedrock_kb_retrieval_mcp_server; print(f'bedrock_kb_retrieval_mcp_server module found at: {awslabs.bedrock_kb_retrieval_mcp_server.__file__}')" 2>/dev/null
    if [ $? -ne 0 ]; then
        echo "Still cannot import bedrock_kb_retrieval_mcp_server module. Tests may fail."
...
# $PYTEST_CMD --cov=awslabs.bedrock_kb_retrieval_mcp_server --cov-report=term-missing tests/

# If you want to run with coverage and generate an HTML report, uncomment the following line
# $PYTEST_CMD --cov=awslabs.bedrock_kb_retrieval_mcp_server --cov-report=html tests/
