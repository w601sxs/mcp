...
from awslabs.iam_mcp_server.errors import handle_iam_error, IamClientError, IamValidationError
...
async def create_user(
    ...
    try:
        ...
        result = CreateUserResponse(
            user=user_details,
            message=f'Successfully created user: {user_name}'
        )
...
async def list_policies(
    ...
    path_prefix: Optional[str] = Field(
        description='Path prefix to filter policies', default=None
    ),
    max_items: int = Field(
    ...
    try:
        ...
        if context_entries:
            kwargs['ContextEntries'] = [
                {'ContextKeyName': k, 'ContextKeyValues': [v]}
                for k, v in context_entries.items()
            ]
...
def main():
    ...
    parser.add_argument(
        '--region',
        help='AWS region to use for operations'
    )
