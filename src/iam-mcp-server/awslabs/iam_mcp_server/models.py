...
class IamUser(BaseModel):
    ...
    password_last_used: Optional[str] = Field(None, description="The date and time when the user's password was last used")
...
class IamRole(BaseModel):
    ...
    assume_role_policy_document: Optional[str] = Field(None, description="The trust policy document")
    description: Optional[str] = Field(None, description="The description of the role")
    max_session_duration: Optional[int] = Field(None, description="Maximum session duration in seconds")
...
class IamPolicy(BaseModel):
    ...
    default_version_id: str = Field(..., description="The identifier for the default version of the policy")
    attachment_count: int = Field(..., description="The number of entities attached to the policy")
    permissions_boundary_usage_count: int = Field(0, description="The number of entities using this policy as a permissions boundary")
    is_attachable: bool = Field(..., description="Whether the policy can be attached to users, groups, or roles")
...
class UserDetailsResponse(BaseModel):
    ...
    attached_policies: List[AttachedPolicy] = Field(default_factory=list, description="List of attached managed policies")
    inline_policies: List[str] = Field(default_factory=list, description="List of inline policy names")
    groups: List[str] = Field(default_factory=list, description="List of group names the user belongs to")
    access_keys: List[AccessKey] = Field(default_factory=list, description="List of access keys for the user")
...
class SimulationResult(BaseModel):
    ...
    matched_statements: List[Dict[str, Any]] = Field(default_factory=list, description="Statements that matched")
    missing_context_values: List[str] = Field(default_factory=list, description="Context values that were missing")


class PolicySimulationResponse(BaseModel):
    """Response model for policy simulation."""
    
    evaluation_results: List[SimulationResult] = Field(..., description="List of evaluation results")
    is_truncated: bool = Field(False, description="Whether the response is truncated")
    marker: Optional[str] = Field(None, description="Marker for pagination")
